...
|-------------|---------|---------|-------------------------|-----------------------|
| **[CrowdStrike](https://ir.crowdstrike.com/)** <br> - [Stock Info](https://finance.yahoo.com/quote/CRWD) • [SEC Filing](https://www.sec.gov/edgar/browse/?CIK=0001535527) • [Earning Transcripts](https://seekingalpha.com/symbol/CRWD/earnings/transcripts) | [@CrowdStrike](https://twitter.com/CrowdStrike) &#124; [$CRWD](https://twitter.com/search?q=%24CRWD) | [![Endpoint](https://img.shields.io/badge/Endpoint-green.svg)](#endpoint-protection-and-threat-detection) [![ThreatIntel](https://img.shields.io/badge/ThreatIntel-navy.svg)](#threat-intelligence) [![IDR](https://img.shields.io/badge/IDR-crimson.svg)](#incident-detection-and-response) | The CrowdStrike Falcon platform provides Endpoint Detection and Response (EDR) and Extended Detection and Response (XDR) to analyze behavior through Indicators of Attack (IOA) using deep learning models trained on telemetry from over 1 trillion daily events. These models detect adversarial techniques, including malicious PowerShell execution and lateral movement, without relying on signatures.<br><br> Falcon XDR extends detection beyond endpoints, integrating telemetry from networks, identity systems, cloud environments, and third-party security sources to provide cross-domain threat detection and response.<br><br> Falcon also trains supervised learning models on historical attack patterns observed in AWS, Azure, and GCP environments. These models analyze cloud activity logs to detect deviations such as abnormal credential usage (e.g., sudden cross-region access spikes) and misconfigurations violating CIS Cloud Benchmarks. Statistical anomaly detection algorithms compare runtime behavior of cloud-deployed models (e.g., inference latency, payload structure) against baseline profiles to detect unauthorized modifications, such as adversarial tampering of model weights or inference API manipulation.<br><br> ExPRT.AI enhances vulnerability prioritization by correlating CVSS data with real-time threat intelligence from Threat Graph via machine learning algorithms. This approach improves vulnerability prioritization accuracy by 3x compared to CVSS alone.<br><br> Charlotte AI automates security workflows through natural language processing (NLP) and multi-agent architecture, integrating large language models (LLMs) and task-specific agents for EDR query generation and incident summarization. [More](https://www.crowdstrike.com/blog/) 🎯 | 36.1% |
